...
```bash
cd ~/ai-agent-demos/adk_temp_calc_demo
...
### Approach 2: The Flat Directory Structure (Used here in `adk_temp_calc_demo`)
In this project, everything sits in a single, flat directory (`adk_temp_calc_demo`). If you look at the `__init__.py` file here, it is empty (just a comment). 
